...
| # | 标准 | 验证方式 | 状态 |
|---|---|---|---|
| G1-1 | 配真实 Key 能跑通 S1 | 设置页接入表单 → 写入配置 → run S1 → 产出 `research_questions` 且 `llm_usage` 有非 mock 记录 | ⬜ **Key 由用户测试时录入**（本机无凭据、亦无本地兼容端点可替代；链路已通到「真发请求」前一步） |
...
**G1 判定：通过。** 遗留项（不阻塞）：真实 Key 端到端、前端流降级路径的真机触发、断网重连的界面验证、运行中取消（等 Sprint 4 内核支持协作式取消）。
