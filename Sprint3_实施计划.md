...
**执行情况**：第 0 ~ 11 步全部完成，每步一个 commit，提交顺序与本表一致 ——
`e7d3c73`(0) → `3019bda`(1) → `aef9c8a`(2) → `3bf6373`(3) → `4a55a44`(4) → `a2e0e33`(5) → `a0d3eff`(6) → `57f9eb5`(7) → `07529b6`(8) → `a4a68ef`(9) → `cea514e`(10) → `409ef46`(11)。
G1 闸口结论：**通过**（G1-1 待用户录入 Key）。
...
留证全文见 `researchpilot/docs/evidence/G1-闸口结论.md`。

| # | 标准 | 验证方式 | 状态 |
|---|---|---|---|
| G1-1 | 配真实 Key 能跑通 S1 | 设置页接入表单 → 写入配置 → run S1 → 产出 `research_questions` 且 `llm_usage` 有非 mock 记录 | ⬜ **实测未通过（2026-09-20 复查）**：真实配置副本 + 关 `llm_cache`（确保真出网）跑 S1，1.5s 失败于 `OpenRouter HTTP 401: Missing Authentication header`。根因是凭据管理器里 `OpenRouter` 那条 Key **只有 8 字符**（真实 Key 约 70），而健康探测打的是公开的 `GET /models`，**配假 Key 也显示「可用」**。本轮已补「已录入·存疑」提示（`426c5d1`）并把留证脚本固化为 `tmp/smoke_g1_real_s1.py`；**待用户重新录入真实 Key 后复跑该脚本收口** |
| G1-2 | 30 秒以上任务全程有实时进度 | `ai.providers.mock.delay_ms=30000` 真跑 S1，看 SSE 首帧与终帧的间隔 | ✅ **30070ms 全程，首帧 +29ms（全程 0.1%）**，26/26 检查通过；浏览器侧另有一轮 8/8 |
| G1-3 | 预算批准后真能恢复 | 已由 `c418338` 实现；本次在**异步路径**上重跑 | ✅ `tests/test_budget_grants.py` 9/9，含「受理 → `job_id` → paused → 批准 → succeeded」且无手工改额度 |
| — | 回归全绿 | `uv run pytest`（≥110）+ `npm run lint` + `npm run build` | ✅ pytest **167 passed** · ruff clean · oxlint **0 error** · `tsc -b && vite build` 成功 |
| — | 迁移安全 | 在真实库副本上 `upgrade head` 后行数不变（`shutil.copy2` 绝对路径法） | ✅ 副本（停在 Sprint 2 末 `246a42e41036`）连升 3 个迁移到 head，5 张新表出现、**老表逐表行数一字不差**，真实库未被改动 |

**G1 判定：通过（G1-1 除外）。** G1-1 是本闸口唯一未闭环项，原因已查明（凭据无效，
非链路缺陷），处置见上表与《Sprint 4 实施计划》的「起点」一节。
其余遗留项（不阻塞）：真实 Key 端到端、前端流降级路径的真机触发、断网重连的界面验证、
运行中取消（等 Sprint 4 内核支持协作式取消）。

**2026-09-20 补充（接入链路收口 + 3 项遗漏）**：G1 通过后，针对用户反馈的
「接真实模型后跑不出结果 / 供应商统计里没有该请求」等 7 个问题完成修复
（`7765fd6` ~ `a04bc9c`），并补齐三处遗漏 —— 路由候选 trim 与模型清单校验（`10ac7e3`）、
「被引用」列如实反映 Agent 档位归属（`298f168`）、用量统计支持全局汇总并在界面上可见
（`b005f71`），以及凭据存疑提示（`426c5d1`）。回归 **202 passed** · ruff clean ·
`tsc -b && vite build` 成功。
